...
- [ ] マスタ管理: 追加したあとにリストが更新されない
- [ ] サブタスクを追加すると、あとから編集できない
- [ ] スマホからだと、期限設定フィールドの背景がグレーになっている（白でよい）
- [ ] スマホからだと、期限設定フィールドが右側にはみ出している
- [ ] 配布タスクの配布者名が「？」になることがある
- [ ] PC版: タスク完了の丸枠ボタンのクリック判定が厳しく、タスク詳細が開いてしまうことが多い（判定範囲を広く）

### UX改善
- [ ] 添付ファイルのプレビューがしたい
- [ ] 不服で差し戻されたタスクは、タスク作成者のタスクトレーに返ってくるほうが自然
- [ ] レポートの右上ボタンはアイコンではなく「レポートを閉じる」テキストにする
- [ ] レポートでは完了したタスクは集計に含めない
- [ ] タスクの削除はタスク作成者だけが実行可能にする
- [ ] マスタ管理: トップページの「+追加」は不要（マスタ管理画面から常に追加するため）
- [ ] 「不服申し立て」→「差し戻し」に表記変更
- [ ] タスクの「×」ボタン横のアイコンは不要
- [ ] マスタ管理: 日付順に並べる
- [ ] マスタ管理: 月次と年次で表示切り替え
- [ ] 「×」ボタン押下時に削除確認ダイアログを表示する
- [ ] 完了済みタスクの丸枠に「☑️」を表示する
...
- [ ] 完了済みタスクをまとめて「処理済み」ボックスに移動するボタンを画面上部に追加
- [ ] PC版3カラムレイアウト（左: タスクトレー／中: 4象限タスク／右: 定型タスク）
- [ ] タスクに「領域（タグ）」を設定できるようにする
  - 領域タグは管理者権限を持つユーザーが「領域マスタ」から追加・編集・削除
  - DBに `areas` テーブル（id, name, color など）と `tasks.area_id` カラムを追加
